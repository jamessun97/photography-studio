...
export async function GET(){
 ...
 try{const row=await database().prepare('SELECT data, revision FROM study_states WHERE user_id = ?').bind(user.userId).first<{data:string;revision:number}>();return reply(row?{state:JSON.parse(row.data),revision:row.revision}:{state:emptyState,revision:0});}catch{return reply({error:'档案暂时无法读取，请稍后重试。'},503);}
}
export async function PUT(request:Request){
 ...
 try{
  ...
  const updated=revision===0
   ? await database().prepare('INSERT INTO study_states (user_id,data,revision,updated_at) VALUES (?,?,1,?) ON CONFLICT(user_id) DO NOTHING').bind(user.userId,JSON.stringify(state),now).run()
   : await database().prepare('UPDATE study_states SET data=?, revision=revision+1, updated_at=? WHERE user_id=? AND revision=?').bind(JSON.stringify(state),now,user.userId,revision).run();
