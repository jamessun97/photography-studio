import { validLearning,type LearningProgress } from './learning-progress';
export type Practice = { lessonId?:number; syllabusVersion?:string; sourcePracticeId?:string; id:string; module:number; date:string; hours:number; title:string; intention:string; evidence:string; reflection:string; feedback:string; next:string; status:string };
export type Project = {id:string;title:string;question:string;sequence:string;feedback:string;next:string};
export type StudyState = {learning?:LearningProgress;week:number; equipment:string; records:Practice[]; projects:Project[]};
